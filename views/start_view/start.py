...
def formatted_price(price):
    try:
        if price:
            if price == 'Frete Grátis' or price == 'Grátis':
                return 0.0
            else:
                return price.replace('R$', '').replace('ou', '').strip().replace('.', '').replace(',', '.')
        else:
            return 0.0
    except:
        return 0.0
...
def scrape_price(driver, link, xpaths, site, prices_dict, prices_freight, freight_actions=None, stock_xpath=None):
    ...
    try:
        ...
        if stock_xpath:
            try:
                wait_for_element(driver, By.XPATH, stock_xpath, timeout= 2)
...
def main():
    xpaths = [
        ...
        },
        {
            'site': 'Kabum',
            'xpaths_price': {
                'price_in_cash': ['//div[@id= "blocoValores"]/div[2]//h4', '//div[@id= "blocoValores"]/div[3]//h4'],
            ...
            'xpaths_freight': [
                ...
                lambda driver: wait_for_element(driver, By.XPATH, '//div[@id= "listaOpcoesFrete"]/div[span[text()= "Entrega Econômica" or text()= "Padrão"]]/span[2]').text
                ] 
        },
        {   
            'site': 'Terabyte',
    ...
    try:
        ...
        for tipo, descricao in tipos.items():
            ...
            if lojas_validas:
                ...
                result.append(
                    f'Melhor preço {descricao} é na loja {melhor_loja} por R$ {melhor_preco:.2f}\n'
                )
            else:
                result.append(f'Sem preços {descricao} disponíveis.')

        # Salvamento automático (você pode ajustar isso para condicional se quiser)
        try:
            ...
            for site, price_data in prices.items():
                db.save_history(
                    product_id,
                    site,
                    price_data.get('price_in_cash', 0.0),
                    price_data.get('price_full', 0.0),
                    freight.get(site, {}).get('frete', 0.0)
                )
        except Exception as e:
